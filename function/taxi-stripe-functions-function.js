...
const TAXI_CARDS_BUCKET = process.env.TAXI_CARDS_BUCKET;
...
export async function createStripeToken(change) {
    Bucket.initialize({ apikey: `${SECRET_API_KEY}` });
    const cardData = change.current;
    ...
    // add token in TAXI_cards_bucket
    cardData.card_token = token.id;
    await Bucket.data.update(TAXI_CARDS_BUCKET, cardData._id, cardData).catch(error => {
        console.log("ERROR WHILE UPDATED CARD DATA", error);
    });

    const user = await Bucket.data.get(TAXI_USER_BUCKET, cardData.user).catch(error => {
    ...
    const customer = await Bucket.data
        .insert(CUSTOMER_BUCKET_ID, customerData)
        .catch(err => console.log(`Create Customer ERROR: ${err}`));
    ...
    await Bucket.data
        .insert(CARD_BUCKET_ID, newCardData)
        .catch(err => console.log(`Create Stripe Card ERROR: ${err}`));
